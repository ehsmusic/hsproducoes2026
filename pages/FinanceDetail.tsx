...
// Fix: Added missing icon imports Wallet and ArrowUpRight
import { 
...
const FinanceDetail: React.FC = () => {
  ...
  // Fix: Defined missing isContratante variable
  ...
  useEffect(() => {
    ...
    // Listener para Subcoleção de Movimentações
  ...
  // Função Crucial: Recalcula os totais no documento pai
  const recalculateFinance = async () => {
    if (!id || !finance) return;
    const snap = await getDocs(collection(db, 'financeiro', id, 'movimentacoes'));
    const totalPago = snap.docs.reduce((acc, curr) => acc + (curr.data().valorMovimentacao || 0), 0);
    const novoSaldo = Math.max(0, finance.valorEvento - totalPago);
  ...
  const handleDeleteMov = async (movId: string) => {
    if (!id || !window.confirm("Deseja excluir este registro de pagamento?")) return;
    try {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40">
      ...
      <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Sincronizando Fluxo...</p>
    </div>
  );
  ...
  if (!event || !finance) return (
    <div className="text-center py-40 space-y-6">
      ...
      <p className="text-slate-500 font-bold">Dados financeiros indisponíveis.</p>
      <button onClick={() => navigate(-1)} className="text-blue-500 font-black uppercase text-xs tracking-widest">Voltar</button>
    </div>
  );

  const progress = Math.min(Math.round((finance.valorPago / finance.valorEvento) * 100), 100);
    ...
    <div className="space-y-10 animate-fade-in pb-24 px-1 md:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-1">
            <span className="px-3 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full text-[9px] font-black uppercase tracking-widest">Painel Financeiro</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
          ...
          {/* Card Resumo Principal com Status Anti-Quebra */}
          <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-12 shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-row justify-between items-center gap-4 mb-12">
              <div className="min-w-0">
                <p className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2 truncate">Valor do Contrato</p>
              ...
              <div className="flex-shrink-0">
                <span className={`inline-flex whitespace-nowrap px-3 py-1.5 md:px-5 md:py-2 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest border transition-all ${
            ...
            <div className="relative z-10 space-y-5">
              <div className="flex items-center justify-between text-[9px] md:text-[10px] font-black uppercase tracking-widest">
                <span className="text-slate-500">Progressão de Caixa</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center space-x-2 text-slate-500 overflow-hidden">
                  ...
                  <span className="text-[8px] md:text-[9px] font-black uppercase truncate">R$ {finance.valorPago.toLocaleString('pt-BR')} Pago</span>
                </div>
          ...
          {/* TABELA DE MOVIMENTAÇÕES - O GRANDE DIFERENCIAL */}
            ...
            <div className="p-8 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-slate-950/20">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Histórico de entradas financeiras</p>
                </div>
              </div>
                  ...
                  {movimentacoes.length > 0 ? movimentacoes.map((mov) => (
                    <tr key={mov.id} className="hover:bg-slate-800/30 transition-colors group">
                      <td className="px-8 py-5">
                        <span className="text-xs font-bold text-slate-300">{new Date(mov.dataMovimentacao).toLocaleDateString('pt-BR')}</span>
                      </td>
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center justify-center space-x-2">
                          {isAdmin && (
                            <>
                              <button 
                                onClick={() => { 
                                  setEditingMov(mov); 
                                  setFormData({ dataMovimentacao: mov.dataMovimentacao, valorMovimentacao: mov.valorMovimentacao, shapePgmt: mov.formaPgmt }); 
                                  setIsModalOpen(true); 
                                }}
                          ...
                          {!isAdmin && <CheckCircle2 size={16} className="text-emerald-500/50" />}
                        </div>
                      </td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan={4} className="px-8 py-20 text-center">
                         <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Nenhum pagamento registrado ainda.</p>
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {/* Custos Operacionais (Somente Admin) */}
          {isAdmin && (
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-white">{new Date(event.dataEvento).toLocaleDateString('pt-BR')}</p>
                </div>
              </div>
              <div className="flex items-start space-x-4">
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-[2.5rem] p-8 space-y-4 shadow-2xl">
             ...
             <h4 className="text-sm font-black text-white uppercase tracking-tighter">Auditoria Financeira</h4>
             <p className="text-[11px] text-slate-500 font-medium leading-relaxed">
               Cada registro nesta página gera um log automático para o fechamento mensal da HS Produções.
             </p>
          </div>
        </div>
      </div>

      {/* MODAL PARA LANÇAMENTO DE MOVIMENTAÇÃO */}
      {isModalOpen && (
          ...
          <div className="relative bg-slate-900 border border-slate-800 rounded-[3rem] w-full max-w-xl shadow-2xl overflow-hidden animate-fade-in my-auto">
            ...
            <form onSubmit={handleSaveMovimentacao} className="p-8 space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Data do Pagamento</label>
                <input 
                  type="date" 
                  required
              ...
              <div className="pt-6 flex gap-4">
                ...
                >
                  ...
                  <span>{editingMov ? 'Atualizar Lançamento' : 'Efetivar Pagamento'}</span>
                </button>
              </div>
            </form>
          </div>
